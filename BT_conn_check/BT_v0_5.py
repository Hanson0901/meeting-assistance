...
class SimpleAgent:
    ...
    def __init__(self, bus, path="/test/agent", capability="DisplayYesNo"):
        self.bus = bus
        self.path = path
    ...
    def Release(self):
        print("ℹ️ Agent released")


    def RequestPinCode(self, device):
        print(f"\n🔐 Device {device} requests PIN code")
        pin = input("Enter PIN code: ").strip()
        return pin


    def DisplayPinCode(self, device, pincode):
        print(f"\n🔐 DisplayPinCode for {device}: PIN={pincode}")
        print("Please enter this PIN on the remote device.")


    def RequestPasskey(self, device):
        print(f"\n🔐 Device {device} requests passkey")
    ...
    def DisplayPasskey(self, device, passkey, entered):
        print(f"\n🔐 DisplayPasskey for {device}: {passkey:06d} (entered={entered})")


    def RequestConfirmation(self, device, passkey):
        print(f"\n🔐 Confirm pairing with {device}")
    ...
    def AuthorizeService(self, device, uuid):
        print(f"\n🔐 Authorize service {uuid} for {device}")
        resp = input("Authorize? (yes/no): ").strip().lower()
        return resp == "yes"


    def Cancel(self):
        print("\nℹ️ Pairing canceled by remote or BlueZ")
...
class BlueZBondingManager:
    ...
    def setup_agent(self):
        try:
            self.agent = SimpleAgent(self.bus, "/test/agent", capability="DisplayYesNo")
            return True
        except Exception as e:
    ...
    def pair_device(self, dev_path):
        try:
            ...
            print("🔐 Start pairing, watch terminal for PIN / confirmation...")
            dev.Pair()
