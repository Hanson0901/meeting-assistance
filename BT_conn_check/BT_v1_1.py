...
class BLEProximityApp:
    ...
    def monitor(self):
        """監控白名單設備 - 結合 BLE 掃描（RSSI）和 l2ping（可達性）"""
        if not self.whitelist:
            print("📋 Whitelist empty")
            return
        
        print("\n=== Proximity monitor (BLE + l2ping) ===")
        print("💡 BLE scan for RSSI, l2ping for reachability")
        print("💡 Works with Android even when not advertising")
        print("⚠️  Devices should be PAIRED for best results")
        ...
        try:
            while True:
                ...
                # 第一步：快速 BLE 掃描獲取 RSSI（3秒）
                print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Cycle {cycle_count}")
                print("📡 BLE scan for RSSI...", flush=True)
                ble_results = self.manager.scan_devices_quick(timeout=3, target_macs=whitelist_macs)
                
                # 第二步：對沒有 BLE 信號的已配對設備使用 l2ping
                devices_need_ping = []
                for d in self.whitelist:
                    mac = d["mac"]
                    if mac not in ble_results or not ble_results[mac]['is_online']:
                        if self.manager.is_bonded(mac):
                            devices_need_ping.append(mac)
                
                l2ping_results = {}
                if devices_need_ping:
                    print(f"📍 Checking {len(devices_need_ping)} devices with l2ping...", flush=True)
                    for mac in devices_need_ping:
                        reachable, rtt = self.manager.check_device_reachable_l2ping(mac, timeout=2)
                        l2ping_results[mac] = {"reachable": reachable, "rtt": rtt}
                ...
                for d in self.whitelist:
                    ...
                    # 獲取當前狀態
                    has_ble = mac in ble_results and ble_results[mac]['is_online']
                    has_l2ping = mac in l2ping_results and l2ping_results[mac]['reachable']
                    ...
                    if has_ble:
                        ...
                        print(f"{d['name']:<12} ({mac}): {status:<15} RSSI={rssi:>4} dBm [BLE]")
                        ...
                        print(f"{d['name']:<12} ({mac}): {status:<15} RSSI={rssi_display} dBm [L2PING RTT:{rtt:.1f}ms]")
                        ...
                        if was_online:
                            status = "🔴 JUST LOST"
                            rssi_display = f"(was {last_state['rssi']})" if last_state['rssi'] and last_state['rssi'] != -127 else "N/A"
...
def main_menu():
    ...
    while True:
        ...
        print("   (BLE RSSI + l2ping hybrid mode)")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("⚠️ l2ping not found. Install: sudo apt install bluez-tools")
    ...
    print("="*70)
    print(" BlueZ Bluetooth Proximity Monitor (Hybrid Mode)")
    print(f" Whitelist: {WHITELIST_FILE}")
    print(f" Bonding keys: {BONDING_KEYS_DIR}")
    print("\n 💡 BLE scan for RSSI + l2ping for Android compatibility")
