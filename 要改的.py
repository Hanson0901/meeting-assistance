...
【改進】採用舊版本 prompt + 新統計輸出 + 類型聚合迴圈 + CSV 產出
核心優化：for 迴圈按類型連續執行 + processing_status 即時更新 + 專業 prompt + 統計輸出 + CSV 詳細報告
"""
...
class LlamaCppQwen3Extractor:
    ...
    def __init__(self, model_path="../Qwen3-4B-Q8_0.gguf"):
        ...
        print("   ✓ 【新】舊版本 prompt（專業風格）")
        print("   ✓ 【新】統計輸出（人物、主題、決策、行動）")
        print("   ✓ 【新】CSV 詳細報告")
    ...
    def generate_response(self, prompt, max_tokens=250, retry_count=0):
        ...
        try:
            ...
            if "out of memory" in str(e).lower():
                print(f"    記憶體不足 (重試 {retry_count + 1}/{self.max_retries})")
                self.aggressive_memory_cleanup()
                time.sleep(1)
    ...
    def extract_all_people_loop(self, segments: List[Dict], session_id: str = None, processing_status: dict = None) -> Dict:
        ...
        for idx, seg in enumerate(segments, 1):
            ...
            if session_id and processing_status:
                progress = 60 + int(8*(idx / total_segments))
...
請開始總結："""
    ...
    def process_srt_file_aggregated(self, srt_file_path, output_file_path=None, max_duration=120, max_chars=2000, session_id: str = None, processing_status: dict = None):
        ...
        用 for 迴圈按類型連續執行 + processing_status 進度追蹤 + 舊版本 prompt + 統計輸出 + CSV 產出
        """
        try:
            ...
            if output_file_path is None:
                ...
                csv_file_path = f'{base_name}{timestamp}.csv'
                summary_file_path = f'{session_id}_summary{timestamp}.md'
            else:
            ...
            with open(summary_file_path, 'w', encoding='utf-8') as f:
                f.write("# 會議整體主題總結（Pi 5 16GB llama.cpp GGUF Q8_0 版本 v5.5 類型聚合迴圈版）\n\n")
...
if __name__ == "__main__":
    ...
    print("   Raspberry Pi 5 16GB 優化版本 v5.5 (類型聚合迴圈版 + 舊版本 prompt + 統計輸出 + CSV)")
    print("="*80)
