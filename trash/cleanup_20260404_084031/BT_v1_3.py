...
class BlueZBondingManager:
    ...
    def scan_devices_quick(self, timeout=3, target_macs=None):
        ...
        返回: {mac: {"rssi": int, "name": str, "is_online": bool, "connected": bool}}
        """
        ...
        device_rssi_history = {}
        scan_iterations = max(1, int(timeout / 0.5))
        ...
        try:
            for i in range(scan_iterations):
                ...
                for path, ifaces in objects.items():
                    ...
                    if addr and rssi != -127:
                        device_rssi_history.setdefault(addr, []).append(rssi)
    ...
    def monitor(self):
        ...
        try:
            while True:
                ...
                reachable_macs = [mac for mac, r in l2ping_results.items() if r["reachable"]]
                ...
                for d in self.whitelist:
                    ...
                    l2_ok = l2ping_results.get(mac, {}).get("reachable", False)
                    l2_rtt = l2ping_results.get(mac, {}).get("rtt", 0.0)

                    ble_info = ble_results.get(mac)
                    has_ble_rssi = bool(ble_info and ble_info.get("is_online", False))

                    last_state = device_last_state.get(
                        mac, {"online": False, "rssi": -127, "timestamp": 0}
                    )
                    ...
                    if l2_ok:
                        if has_ble_rssi:
                            ...
                            print(
                                f"{d['name']:<12} ({mac}): {status:<15} "
                                f"RSSI={rssi:>4} dBm [L2PING✓ BLE✓]"
                            )
                            ...
                            status = "🟢 RECONNECTED" if not was_online else "🟢 ONLINE"
                            rssi_display = f"{rssi:>4}" if rssi != -127 else " N/A"
                            print(
                                f"{d['name']:<12} ({mac}): {status:<15} "
                                f"RSSI={rssi_display} dBm [L2PING✓ RTT:{l2_rtt:.1f}ms]"
                            )
                            device_last_state[mac] = {
                                "online": True,
                                "rssi": rssi,
                        ...
                        if was_online:
                            ...
                            print(
                                f"{d['name']:<12} ({mac}): {status:<15} "
                                f"RSSI=N/A [Offline {offline_duration:.0f}s]"
                            )
...
if __name__ == "__main__":
    if os.geteuid() != 0:
        ...
        print("   Run with: sudo python3 BT_v1_2.py")
        sys.exit(1)
