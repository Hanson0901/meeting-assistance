...
# -*- coding: utf-8 -*-
"""
Qwen/Qwen3-14B-AWQ 會議記錄整理助手 (模塊化版本)
專門用於處理CSV文件中的會議記錄並進行逐行重點整理，最後總結整個會議主題
模塊化版本：將出現人物、核心要點、決策事項、行動項目、總結分別處理
"""
import torch
...
class Qwen3MeetingRecordExtractor:
    """
    使用Qwen/Qwen3-4B-Instruct-2507模型專門進行會議記錄整理的助手類（模塊化版本）
    """
    ...
    def process_csv_file(self, csv_file_path, output_file_path=None):
        ...
        try:
            ...
            if output_file_path is None:
                ...
                output_file_path = f'meeting_summary_modular_{timestamp}.csv'
                summary_file_path = f'meeting_overall_summary_modular_{timestamp}.txt'
            else:
