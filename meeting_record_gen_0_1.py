...
class QwenModelInterface:
    ...
    def _fallback_generation(self, prompt: str) -> str:
        ...
        if "會議主題" in prompt or "meeting topic" in prompt_lower:
            ...
            # 生成較長的發言內容
            detailed_speeches = [
                "從我們目前的市場分析來看，這個項目具有很高的戰略價值。首先，它能夠幫助我們提升核心競爭力，其次可以優化我們的業務流程，最重要的是能夠為客戶創造更大的價值。我建議我們從三個方面來推進：技術層面的創新、流程的標準化，以及團隊能力的提升。",
                "根據我們的財務分析報告，這個方案的投資回報率預計在18個月內達到正值。雖然初期投入較大，但長期效益非常明顯。我們需要考慮的風險因素包括市場變化、技術更新速度，以及競爭對手的策略調整。建議我們制定階段性的里程碑和風險控制機制。",
                "從客戶需求的角度來分析，我們發現有三個關鍵趨勢值得關注。第一是個性化需求的增加，第二是對服務效率的更高要求，第三是對數據安全和隱私保護的重視。我們的解決方案必須能夠同時滿足這三個方面的需求，才能在市場上獲得競爭優勢。",
                "技術實施方面，我們需要考慮系統的可擴展性、穩定性和安全性。建議採用微服務架構，這樣可以提高系統的靈活性和維護性。同時，我們還需要建立完整的監控和警報機制，確保系統運行的穩定性。在安全方面，建議實施多層次的安全策略，包括網路安全、數據加密和身份驗證。"
            ]
...
class EnhancedMeetingRecordGenerator:
    ...
    def generate_topics(self, count: int = 100) -> List[str]:
        ...
        for i in range(count):
            topic_prompt = """請生成一個會議主題，要求：
1. 長度控制在12-28個繁體中文字符
2. 直接返回會議主題，不要添加引號或其他格式
3. 主題不必限制與數位相關，可以涵蓋各行各業與領域
...
詳細發言："""
        ...
        try:
            ...
            # 返回備用內容
            return f"感謝大家的關注。關於{context}這個議題，我認為我們需要從多個角度來分析。首先是市場趨勢，其次是技術可行性，最後是實施成本。建議我們制定詳細的執行計劃，並設立階段性目標來確保項目的順利推進。"
    ...
    def generate_extended_meeting_dialogue(self, topic: str, characters: List[Dict], todo_items: List[Dict]) -> List[Dict[str, str]]:
        ...
        dialogue.append({
            "speaker": moderator,
            "content": f"各位同事大家好，歡迎參加今天的重要會議。今天我們要深入討論的主題是「{topic}」，這個議題對我們公司的未來發展具有重要的戰略意義。在開始正式討論之前，我想先簡單說明今天會議的流程安排。我們將分為四個部分：首先是各部門代表的詳細報告，接著是針對具體執行方案的深入討論，然後是風險評估與資源配置，最後是決策總結和後續行動計劃。希望各位能夠積極參與討論，共同為這個項目的成功實施貢獻智慧和力量。"
        })
    ...
    def generate_all_meetings(self, topics: List[str]) -> None:
        ...
        print(f" 開始批量生成 {total_topics} 個詳細會議記錄（目標每個10000字+）...")
        print("=" * 60)
        ...
        print("=" * 60)
        print(f" 批量生成完成！")
        print(f" 成功生成: {success_count}/{total_topics} 個詳細會議記錄")
